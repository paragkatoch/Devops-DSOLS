...
func ProductController(storage storage.Storage, cfg *config.Config) {
	...
	router.HandleFunc("POST /api/product", pt.Instrument(handler.CreateProduct(publish), "product", "POST /api/product"))
	...
	router.HandleFunc("POST /api/product/quantity", pt.Instrument(handler.UpdateProductQuantity(publish), "product", "POST /api/product/quantity"))
